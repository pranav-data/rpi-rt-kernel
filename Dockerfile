...
RUN export PATCH=$(curl -s https://mirrors.edge.kernel.org/pub/linux/kernel/projects/rt/${LINUX_KERNEL_VERSION}/ | sed -n 's:.*<a href="\(.*\).patch.gz">.*:\1:p' | sort -V | tail -1) && \
    ...
    gzip ${PATCH}.patch.gz | patch -p1 --verbose
...
RUN export DATE="2023-02-22" && \
    export RASPIOS="2023-02-21-raspios-bullseye-arm64-lite.img" && \
    echo "Downloading ${RASPIOS}.xz" && \
    curl https://downloads.raspberrypi.org/raspios_lite_arm64/images/${RASPIOS_IMAGE_NAME}-${DATE}/${RASPIOS}.xz --output ${RASPIOS}.xz && \
    xz -d ${RASPIOS}.xz

## Downloads latest Bookworm release
